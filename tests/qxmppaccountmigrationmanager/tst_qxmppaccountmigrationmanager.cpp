...
static std::unique_ptr<TestClient> newClient(bool withManagers, bool disableAutoReset = false)
{
    auto client = std::make_unique<TestClient>(false, disableAutoReset);
...
{
    auto client = newClient(true, true);
    auto *manager = client->findExtension<QXmppAccountMigrationManager>();
    ...
    client->expect(u"<iq id='qxmpp6' to='mix2@gamer.com' type='get'>"
                   "<pubsub xmlns='http://jabber.org/protocol/pubsub'>"
                   "<items node='urn:xmpp:mix:nodes:participants'/>"
                   "</pubsub>"
                   "</iq>"_s);
    client->inject(u"<iq id='qxmpp6' from='mix2@gamer.com' type='result'>"
                   "<pubsub xmlns='http://jabber.org/protocol/pubsub'>"
                   "<items node='urn:xmpp:mix:nodes:participants'>"
                   "<item id='mix2BareId'>"
    ...
    client->expect(u"<iq id='qxmpp10' type='set'>"
                   "<query xmlns='jabber:iq:roster'>"
                   ...
                   "</query>"
                   "</iq>"_s);
    client->inject(packetToXml(newRoster(client.get(), 1, "qxmpp10", QXmppIq::Result, 0)));

    client->expect(u"<iq id='qxmpp11' to='pasnox@xmpp.example' type='set'>"
                   ...
                   "</client-join>"
                   "</iq>"_s);
    client->inject(u"<iq id='qxmpp11' type='result'>"
    ...
    client->expect(u"<iq id='qxmpp3' to='pasnox@xmpp.example' type='set'>"
                   "<vCard xmlns='vcard-temp'>"
...
void tst_QXmppAccountMigrationManager::testSerialization()
{
    ...
    const QByteArray xml2 =
        ...
        "<item xmlns=\"jabber:iq:roster\" jid=\"mix2@gamer.com\" name=\"Mix 2 Gamer\"><channel xmlns=\"urn:xmpp:mix:roster:0\" participant-id=\"mix2BareId\"/></item>"
        "<mix channel=\"mix2@gamer.com\" nick=\"Joe @ Mix 2 Gamer\"/>"
        "</roster>"
        "<vcard>"
        "<vCard xmlns=\"vcard-temp\">"
        "<NICKNAME>It is me Bookri</NICKNAME>"

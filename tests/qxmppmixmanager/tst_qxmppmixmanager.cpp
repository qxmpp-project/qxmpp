...
void tst_QXmppMixManager::testHandlePubSubEvent()
{
    ...
    QSignalSpy userJoinedOrParticipantModifiedSpy(&manager, &QXmppMixManager::userJoinedOrParticipantModified);
    ...
    QCOMPARE(userJoinedOrParticipantModifiedSpy.size(), 2);
    for (auto i = 0; i < userJoinedOrParticipantModifiedSpy.size(); i++) {
        const auto &arguments = userJoinedOrParticipantModifiedSpy.at(i);
        QCOMPARE(arguments.at(0).toString(), channelJid);

...
const auto NODE_CONFIG_FORM_TYPE = QStringLiteral(u"http://jabber.org/protocol/pubsub#node_config");
const auto PUBLISH_OPTIONS_FORM_TYPE = QStringLiteral("http://jabber.org/protocol/pubsub#publish-options");

const auto ACCESS_MODEL = QStringLiteral("pubsub#access_model");
const auto BODY_XSLT = QStringLiteral("pubsub#body_xslt");
const auto CHILD_ASSOCIATION_POLICY = QStringLiteral("pubsub#children_association_policy");
const auto CHILD_ASSOCIATION_ALLOWLIST = QStringLiteral("pubsub#children_association_whitelist");
const auto CHILD_NODES = QStringLiteral("pubsub#children");
const auto CHILD_NODES_MAX = QStringLiteral("pubsub#children_max");
const auto COLLECTIONS = QStringLiteral("pubsub#collection");
const auto CONTACT_JIDS = QStringLiteral("pubsub#contact");
const auto DATA_FORM_XSLT = QStringLiteral("pubsub#dataform_xslt");
const auto NOTIFICATIONS_ENABLED = QStringLiteral("pubsub#deliver_notifications");
const auto INCLUDE_PAYLOADS = QStringLiteral("pubsub#deliver_payloads");
const auto DESCRIPTION = QStringLiteral("pubsub#description");
const auto ITEM_EXPIRY = QStringLiteral("pubsub#item_expire");
const auto NOTIFICATION_ITEM_PUBLISHER = QStringLiteral("pubsub#itemreply");
const auto LANGUAGE = QStringLiteral("pubsub#language");
const auto MAX_ITEMS = QStringLiteral("pubsub#max_items");
const auto MAX_PAYLOAD_SIZE = QStringLiteral("pubsub#max_payload_size");
const auto NODE_TYPE = QStringLiteral("pubsub#node_type");
const auto NOTIFICATION_TYPE = QStringLiteral("pubsub#notification_type");
const auto CONFIG_NOTIFICATIONS_ENABLED = QStringLiteral("pubsub#notify_config");
const auto NODE_DELETE_NOTIFICATIONS_ENABLED = QStringLiteral("pubsub#notify_delete");
const auto RETRACT_NOTIFICATIONS_ENABLED = QStringLiteral("pubsub#notify_retract");
const auto SUB_NOTIFICATIONS_ENABLED = QStringLiteral("pubsub#notify_sub");
const auto PERSIST_ITEMS = QStringLiteral("pubsub#persist_items");
const auto PRESENCE_BASED_NOTIFICATIONS = QStringLiteral("pubsub#presence_based_delivery");
const auto PUBLISH_MODEL = QStringLiteral("pubsub#publish_model");
const auto PURGE_WHEN_OFFLINE = QStringLiteral("pubsub#purge_offline");
const auto ALLOWED_ROSTER_GROUPS = QStringLiteral("pubsub#roster_groups_allowed");
const auto SEND_LAST_ITEM = QStringLiteral("pubsub#send_last_published_item");
const auto TEMPORARY_SUBSCRIPTIONS = QStringLiteral("pubsub#tempsub");
const auto ALLOW_SUBSCRIPTIONS = QStringLiteral("pubsub#subscribe");
const auto TITLE = QStringLiteral("pubsub#title");
const auto PAYLOAD_TYPE = QStringLiteral("pubsub#type");
...
std::optional<QXmppPubSubPublishOptions> QXmppPubSubPublishOptions::fromDataForm(const QXmppDataForm &form)
{
    ...
    if (!QXmppDataFormBase::fromDataForm(form, publishOptions)) {
        return std::nullopt;
    }
